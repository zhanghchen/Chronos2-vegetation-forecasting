...
# given pixel), then automatically runs the actual Chronos-2 zero-shot
# experiment on that data (Part 7 validation) - no need for a human to be
# watching at the exact moment the download finishes. Resilient to
# environment teardowns the same way run_era5_fetch_supervised.sh is
...
#
# Generalized (was hardcoded to the evergreen pixel). Usage:
#   run_experiment_when_ready.sh <site> <lat> <lon> <year> [<year> ...]
set -e
...
RESULT_MARKER=/home/deh25003/chronos-forecasting/Chronos2-vegetation-forecasting/experiments/global_era5_chronos/results/${SITE}/metrics_era5.txt
LOG=$LOGDIR/era5_experiment_autorun_${SITE}.log

echo "=== autorun[$SITE]: waiting for ERA5 fetch to complete, started at $(date) ===" >> "$LOG"

while [ ! -f "$DONE_MARKER" ]; do
  sleep 60
done

echo "=== autorun[$SITE]: DONE_MARKER found at $(date), running Chronos-2 zero-shot experiment ===" >> "$LOG"

if [ -f "$RESULT_MARKER" ]; then
  echo "=== autorun[$SITE]: result already exists at $RESULT_MARKER, not re-running ===" >> "$LOG"
else
  $PYTHON run_era5_chronos.py --lat "$LAT" --lon "$LON" --site "$SITE" \
      --era5-years "$@" --test-year "$LAST_YEAR" >> "$LOG" 2>&1
  echo "=== autorun[$SITE]: experiment finished at $(date), exit code $? ===" >> "$LOG"
fi
